...
# MAGIC %md
# MAGIC # DDL for consumer.Sales_Dim_Date
...
data_module = "Dim"
table_name = "Date"
full_table_name = "Sales_Dim_Date"
...
table_comment = "Date dimension exposed for reporting"
...
schema = StructType([
    ...
    StructField("Date SID", DataType.fromDDL("BIGINT"), False, metadata={'comment': 'Date surrogate key', 'business_key': True}),
    StructField("Date", DataType.fromDDL("TIMESTAMP"), False, metadata={'comment': 'Calendar date'}),
    StructField("Calendar Year", DataType.fromDDL("STRING"), True, metadata={'comment': 'Calendar year label'}),
    StructField("Month Name", DataType.fromDDL("STRING"), True, metadata={'comment': 'English month name'}),
])
