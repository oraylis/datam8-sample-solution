...
max_raw["Raw_Sales_Order_SalesOrderHeader"] = spark.sql(f"""
SELECT
...
source_delta_1_df = (
    spark.table(f"{catalog.name}.raw.Sales_Order_SalesOrderHeader")
    .filter(F.col("__InsertTimestampUTC") > max_raw["Raw_Sales_Order_SalesOrderHeader"])
    .selectExpr(
        "`SalesOrderID`",
        "`RevisionNumber`",
        "`OrderDate`",
