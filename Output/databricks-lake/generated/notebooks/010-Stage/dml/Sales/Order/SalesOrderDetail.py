...
max_raw["Raw_Sales_Order_SalesOrderDetail"] = spark.sql(f"""
SELECT
...
source_delta_1_df = (
    spark.table(f"{catalog.name}.raw.Sales_Order_SalesOrderDetail")
    .filter(F.col("__InsertTimestampUTC") > max_raw["Raw_Sales_Order_SalesOrderDetail"])
    .selectExpr(
        "`SalesOrderID`",
        "`SalesOrderDetailID`",
        "`OrderQty`",
