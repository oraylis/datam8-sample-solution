...
max_raw["Raw_Sales_Customer_CustomerAddress"] = spark.sql(f"""
SELECT
...
source_delta_1_df = (
    spark.table(f"{catalog.name}.raw.Sales_Customer_CustomerAddress")
    .filter(F.col("__InsertTimestampUTC") > max_raw["Raw_Sales_Customer_CustomerAddress"])
    .selectExpr(
        "`CustomerID`",
        "`AddressID`",
        "`AddressType`",
