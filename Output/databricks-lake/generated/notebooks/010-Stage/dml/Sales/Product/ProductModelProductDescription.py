...
max_raw["Raw_Sales_Product_ProductModelProductDescription"] = spark.sql(f"""
SELECT
...
source_delta_1_df = (
    spark.table(f"{catalog.name}.raw.Sales_Product_ProductModelProductDescription")
    .filter(F.col("__InsertTimestampUTC") > max_raw["Raw_Sales_Product_ProductModelProductDescription"])
    .selectExpr(
        "`ProductModelID`",
