...
max_raw["Raw_Sales_Product_ProductModel"] = spark.sql(f"""
SELECT
...
source_delta_1_df = (
    spark.table(f"{catalog.name}.raw.Sales_Product_ProductModel")
    .filter(F.col("__InsertTimestampUTC") > max_raw["Raw_Sales_Product_ProductModel"])
    .selectExpr(
        "`ProductModelID`",
        "`Name`",
        "`CatalogDescription`",
