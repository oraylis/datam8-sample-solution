...
max_raw["Raw_Sales_Other_Address"] = spark.sql(f"""
SELECT
...
source_delta_1_df = (
    spark.table(f"{catalog.name}.raw.Sales_Other_Address")
    .filter(F.col("__InsertTimestampUTC") > max_raw["Raw_Sales_Other_Address"])
    .selectExpr(
        "`AddressID`",
        "`AddressLine1`",
        "`AddressLine2`",
