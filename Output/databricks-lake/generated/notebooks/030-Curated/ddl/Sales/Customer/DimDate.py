...
schema = StructType([
    ...
    StructField("DateSID", DataType.fromDDL("BIGINT"), False, metadata={'comment': 'Date surrogate key', 'surrogate_key': True}),
...
for f in schema.fields:
    ...
    if md.get("foreign_key"):
        ...
        if ref_table:
            ref_table_base = ref_table.replace(".", "_")
            constraint_name = f"fk_{catalog.name}_{ref_table_base}"
...
# table attributes
table_instance.set_table_tags({'business_area': 'sales', 'write_mode': 'merge', 'jobs': 'daily', 'data_retention': '7_days'})
