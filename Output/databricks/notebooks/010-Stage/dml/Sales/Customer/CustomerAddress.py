...
# MAGIC %md
# MAGIC # DML for bronze.Sales_Customer_CustomerAddress
# MAGIC History configuration of this entity
# MAGIC - __Business Key Columns__: ['CustomerID', 'AddressID']
# MAGIC - __SCD0 columns__: []
# MAGIC - __SCD1 columns__: ['CustomerID', 'AddressID', 'AddressType']
...
# MAGIC %md
# MAGIC #### 'Sales_Customer_CustomerAddress'

# COMMAND ----------

max_external["raw_Sales_Customer_CustomerAddress"] = spark.sql(f"""
SELECT
...
source_delta_1_df = (
    ...
    .filter(F.col("__InsertTimestampUTC") > max_external["raw_Sales_Customer_CustomerAddress"])
    .selectExpr(
        "`CustomerID`",
        "`AddressID`",
        "`AddressType`",
...
latest_snapshot_key_cols = ["CustomerID", "AddressID"]
