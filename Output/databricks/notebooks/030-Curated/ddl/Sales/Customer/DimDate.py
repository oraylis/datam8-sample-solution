...
# MAGIC %md
# MAGIC # DDL for gold.Sales_Customer_DimDate
...
table_comment = "Date dimension with calendar attributes"
...
schema = StructType([
    ...
    StructField("DateSID", DataType.fromDDL("BIGINT"), False, metadata={'comment': 'Date surrogate key'}),
    StructField("DateID", DataType.fromDDL("INT"), False, metadata={'comment': 'Calendar date ID', 'business_key': True}),
    StructField("Date", DataType.fromDDL("TIMESTAMP"), False, metadata={'comment': 'Calendar date'}),
    StructField("CalendarYear", DataType.fromDDL("STRING"), False, metadata={'comment': 'Calendar year label'}),
    StructField("MonthName", DataType.fromDDL("STRING"), False, metadata={'comment': 'English month name'}),
])
...
create_sql = f"""CREATE TABLE IF NOT EXISTS {catalog_name}.{zone}.Sales_Customer_DimDate (
  ...
  `DateSID` BIGINT NOT NULL COMMENT 'Date surrogate key',
  `DateID` INT NOT NULL COMMENT 'Calendar date ID',
  `Date` TIMESTAMP NOT NULL COMMENT 'Calendar date',
  `CalendarYear` STRING NOT NULL COMMENT 'Calendar year label',
  `MonthName` STRING NOT NULL COMMENT 'English month name'
)
USING DELTA
