...
# MAGIC %md
# MAGIC # DML for core.Sales_Other_Address
# MAGIC History configuration of this entity
# MAGIC - __Business Key Columns__: ['_AddressBK'] # noqa: E501
# MAGIC - __SCD0 columns__: [] # noqa: E501
# MAGIC - __SCD1 columns__: ['AddressID', 'AddressLine1', 'AddressLine2', 'CityName', 'CountryRegionName', 'PostalCode', 'StateProvinceName'] # noqa: E501
...
# MAGIC %md
# MAGIC ### Max timestamp for source table '/Stage/Sales/Other/Address'

# COMMAND ----------

max_stage: dict = {}
max_stage["Stage_Sales_Other_Address"] = spark.sql("""
SELECT
  NVL(MAX(__InsertTimestampStageUTC), CAST("1970-01-01" AS TIMESTAMP)) AS MaxStage
FROM `%(catalog)s`.`%(database)s`.`Sales_Other_Address`
WHERE __SourceTable_BK = "%(source_locator)s"
""" % {
    "catalog": catalog.name,
    "database": core_zone,
    "source_locator": "/Stage/Sales/Other/Address",
}).first()[0]
...
# MAGIC %md
# MAGIC ### Delta from source table '/Stage/Sales/Other/Address'
...
""" % {
    "catalog": catalog.name,
    "database": stage_zone,
    "table": "Sales_Other_Address",
    "max_valid_to_date": MAX_VALID_TO_DATE,
    "max_stage_timestamp": max_stage["Stage_Sales_Other_Address"],
    "source_locator": "/Stage/Sales/Other/Address",
})
...
spark.sql("""
MERGE INTO `%(catalog)s`.`%(database)s`.`Sales_Other_Address` AS tgt
...
WHEN MATCHED
  ...
  AND (
    ...
    OR NOT tgt.`AddressLine1` <=> src.`AddressLine1`
    OR NOT tgt.`AddressLine2` <=> src.`AddressLine2`
    OR NOT tgt.`CityName` <=> src.`CityName`
    OR NOT tgt.`CountryRegionName` <=> src.`CountryRegionName`
    OR NOT tgt.`PostalCode` <=> src.`PostalCode`
    OR NOT tgt.`StateProvinceName` <=> src.`StateProvinceName`
  )
THEN UPDATE SET
  ...
  tgt.`CountryRegionName` = src.`CountryRegionName`,
