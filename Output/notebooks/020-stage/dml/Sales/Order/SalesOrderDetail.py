...
SELECT
    ...
    CASE
        WHEN `SalesOrderDetailID` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`SalesOrderDetailID` AS INT) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `SalesOrderDetailID_hasCastError`,
    -- OrderQty
    TRY_CAST(`OrderQty` AS SMALLINT) AS `OrderQty`,
    CASE
        WHEN `OrderQty` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`OrderQty` AS SMALLINT) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `OrderQty_hasCastError`,
    -- ProductID
    ...
    -- UnitPrice
    TRY_CAST(`UnitPrice` AS DECIMAL(19,4)) AS `UnitPrice`,
    CASE
        WHEN `UnitPrice` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`UnitPrice` AS DECIMAL(19,4)) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `UnitPrice_hasCastError`,
    -- UnitPriceDiscount
    TRY_CAST(`UnitPriceDiscount` AS DECIMAL(19,4)) AS `UnitPriceDiscount`,
    CASE
        WHEN `UnitPriceDiscount` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`UnitPriceDiscount` AS DECIMAL(19,4)) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `UnitPriceDiscount_hasCastError`,
    -- LineTotal
    TRY_CAST(`LineTotal` AS DECIMAL(38, 6)) AS `LineTotal`,
    CASE
        WHEN `LineTotal` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`LineTotal` AS DECIMAL(38, 6)) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `LineTotal_hasCastError`,
    -- ModifiedDate
...
WHERE
    ...
    OR SalesOrderDetailID_hasCastError > 0
    OR OrderQty_hasCastError > 0
    OR ProductID_hasCastError > 0
    OR UnitPrice_hasCastError > 0
    OR UnitPriceDiscount_hasCastError > 0
    OR LineTotal_hasCastError > 0
...
if poison_df_count > 0:
    (
        ...
        .mode("append")
        .parquet(f"abfss://{container_name}@{data_lake_name}.dfs.core.windows.net/{stage_zone}/Sales/Order/SalesOrderDetail_poison")
    )
    print(
...
WHERE
    SalesOrderID_hasCastError == 0
    AND SalesOrderDetailID_hasCastError == 0
    AND OrderQty_hasCastError == 0
    AND ProductID_hasCastError == 0
    AND UnitPrice_hasCastError == 0
    AND UnitPriceDiscount_hasCastError == 0
    AND LineTotal_hasCastError == 0
...
spark.sql("""
INSERT INTO `%(catalog)s`.`%(database)s`.`Sales_Order_SalesOrderDetail`
TABLE clean_df
""" % {
