...
SELECT
    ...
    CASE
        WHEN `Description` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`Description` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `Description_hasCastError`,
    -- rowguid
...
FROM typed_table_df
WHERE
    ProductDescriptionID_hasCastError > 0
    OR Description_hasCastError > 0
...
if poison_df_count > 0:
    (
        ...
        .mode("append")
        .parquet(f"abfss://{container_name}@{data_lake_name}.dfs.core.windows.net/{stage_zone}/Sales/Product/ProductDescription_poison")
    )
    print(
...
WHERE
    ProductDescriptionID_hasCastError == 0
    AND Description_hasCastError == 0
...
spark.sql("""
INSERT INTO `%(catalog)s`.`%(database)s`.`Sales_Product_ProductDescription`
TABLE clean_df
""" % {
