...
SELECT
    ...
    CASE
        WHEN `ProductCategoryID` IS NULL THEN  1 -- NOT NULL COLUMN
    ...
    CASE
        WHEN `ParentProductCategoryID` IS NULL THEN 0 -- NULLable COLUMN
        
        WHEN TRY_CAST(`ParentProductCategoryID` AS INT) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `ParentProductCategoryID_hasCastError`,
    -- Name
...
FROM typed_table_df
WHERE
    ProductCategoryID_hasCastError > 0
    OR ParentProductCategoryID_hasCastError > 0
...
if poison_df_count > 0:
    (
        ...
        .mode("append")
        .parquet(f"abfss://{container_name}@{data_lake_name}.dfs.core.windows.net/{stage_zone}/Sales/Product/ProductCategory_poison")
    )
    print(
...
WHERE
    ProductCategoryID_hasCastError == 0
    AND ParentProductCategoryID_hasCastError == 0
...
spark.sql("""
INSERT INTO `%(catalog)s`.`%(database)s`.`Sales_Product_ProductCategory`
TABLE clean_df
""" % {
