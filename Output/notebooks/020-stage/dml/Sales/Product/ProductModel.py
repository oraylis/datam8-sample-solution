...
# MAGIC %md
# MAGIC # DML for stage.Sales_Product_ProductModel
...
SELECT
    ...
    CASE
        WHEN `CatalogDescription` IS NULL THEN 0 -- NULLable COLUMN
        
        WHEN TRY_CAST(`CatalogDescription` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `CatalogDescription_hasCastError`,
    -- rowguid
...
WHERE
    ...
    OR CatalogDescription_hasCastError > 0
...
if poison_df_count > 0:
    (
        ...
        .mode("append")
        .parquet(f"abfss://{container_name}@{data_lake_name}.dfs.core.windows.net/{stage_zone}/Sales/Product/ProductModel_poison")
    )
    print(
...
WHERE
    ...
    AND CatalogDescription_hasCastError == 0
...
spark.sql("""
INSERT INTO `%(catalog)s`.`%(database)s`.`Sales_Product_ProductModel`
TABLE clean_df
""" % {
