...
# MAGIC %md
# MAGIC # DML for stage.Sales_Product_ProductModelProductDescription
...
SELECT
    ...
    CASE
        WHEN `ProductModelID` IS NULL THEN  1 -- NOT NULL COLUMN
    ...
    CASE
        WHEN `ProductDescriptionID` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`ProductDescriptionID` AS INT) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `ProductDescriptionID_hasCastError`,
    -- Culture
    TRY_CAST(`Culture` AS STRING) AS `Culture`,
    CASE
        WHEN `Culture` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`Culture` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `Culture_hasCastError`,
    -- rowguid
...
FROM typed_table_df
WHERE
    ProductModelID_hasCastError > 0
    OR ProductDescriptionID_hasCastError > 0
    OR Culture_hasCastError > 0
...
if poison_df_count > 0:
    (
        ...
        .mode("append")
        .parquet(f"abfss://{container_name}@{data_lake_name}.dfs.core.windows.net/{stage_zone}/Sales/Product/ProductModelProductDescription_poison")
    )
    print(
...
WHERE
    ProductModelID_hasCastError == 0
    AND ProductDescriptionID_hasCastError == 0
    AND Culture_hasCastError == 0
...
spark.sql("""
INSERT INTO `%(catalog)s`.`%(database)s`.`Sales_Product_ProductModelProductDescription`
TABLE clean_df
""" % {
