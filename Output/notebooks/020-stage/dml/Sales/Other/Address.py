...
SELECT
    ...
    CASE
        WHEN `AddressLine1` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`AddressLine1` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `AddressLine1_hasCastError`,
    -- AddressLine2
    TRY_CAST(`AddressLine2` AS STRING) AS `AddressLine2`,
    CASE
        WHEN `AddressLine2` IS NULL THEN 0 -- NULLable COLUMN
        
        WHEN TRY_CAST(`AddressLine2` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `AddressLine2_hasCastError`,
    -- City
    TRY_CAST(`City` AS STRING) AS `City`,
    CASE
        WHEN `City` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`City` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `City_hasCastError`,
    -- StateProvince
    TRY_CAST(`StateProvince` AS STRING) AS `StateProvince`,
    CASE
        WHEN `StateProvince` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`StateProvince` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `StateProvince_hasCastError`,
    -- CountryRegion
    TRY_CAST(`CountryRegion` AS STRING) AS `CountryRegion`,
    CASE
        WHEN `CountryRegion` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`CountryRegion` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `CountryRegion_hasCastError`,
    -- PostalCode
    TRY_CAST(`PostalCode` AS STRING) AS `PostalCode`,
    CASE
        WHEN `PostalCode` IS NULL THEN  1 -- NOT NULL COLUMN
        WHEN TRY_CAST(`PostalCode` AS STRING) IS NULL THEN 1 -- NOT ABLE TO CAST
        ELSE 0 -- ALL GOOD
    END AS `PostalCode_hasCastError`,
    -- rowguid
...
FROM typed_table_df
WHERE
    AddressID_hasCastError > 0
    OR AddressLine1_hasCastError > 0
    OR AddressLine2_hasCastError > 0
    OR City_hasCastError > 0
    OR StateProvince_hasCastError > 0
    OR CountryRegion_hasCastError > 0
    OR PostalCode_hasCastError > 0
...
if poison_df_count > 0:
    (
        ...
        .mode("append")
        .parquet(f"abfss://{container_name}@{data_lake_name}.dfs.core.windows.net/{stage_zone}/Sales/Other/Address_poison")
    )
    print(
...
WHERE
    AddressID_hasCastError == 0
    AND AddressLine1_hasCastError == 0
    AND AddressLine2_hasCastError == 0
    AND City_hasCastError == 0
    AND StateProvince_hasCastError == 0
    AND CountryRegion_hasCastError == 0
    AND PostalCode_hasCastError == 0
...
spark.sql("""
INSERT INTO `%(catalog)s`.`%(database)s`.`Sales_Other_Address`
TABLE clean_df
""" % {
